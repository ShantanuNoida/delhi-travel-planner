...
Agent 2 (Edit Command Agent): applies 20 real edit commands per itinerary
(400 total), cumulatively, via the REAL app code (phase4/intent_classifier +
phase4/edit_engine), cross-checked against phase5/edit_correctness.py --
no simulated/mocked behavior. Starts from the base itinerary Agent 1 already
built (results/base_itinerary_NN.json).

Usage: python run_edits.py [start_id] [end_id]   (1-based, inclusive; default 1 20)
Writes results/edit_itinerary_<id>.json per itinerary as it finishes.
"""
...
PROGRESS_LOG = os.path.join(RESULTS_DIR, "_progress_edits.log")
...
def classify_with_retry(text: str) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        except Exception as e:  # noqa: BLE001 -- QA harness: retry any transient classifier failure
            last_exc = e
...
def run_one(spec: dict) -> dict:
    ...
    steps = []
    for cmd in commands:
        log(f"  [{cmd['n']:02d}/20] ({cmd['category']}/{cmd['probe']}) \"{cmd['command']}\"")
        before = copy.deepcopy(state)
        time.sleep(CALL_SPACING_SEC)
        classification = classify_with_retry(cmd["command"])

        step = {
            "n": cmd["n"], "command": cmd["command"], "category": cmd["category"], "probe": cmd["probe"],
            "expected": cmd["expected"], "classification": classification,
        }
    ...
    result = {
        "spec": spec, "initial_itinerary_full": itinerary, "initial_itinerary_summary": initial_snapshot,
        "final_itinerary_full": state, "final_itinerary_summary": summarize_itinerary(state), "steps": steps,
    }
    out_path = os.path.join(RESULTS_DIR, f"edit_itinerary_{spec['id']:02d}.json")
...
def main():
    ...
    log(f"Team Waypoint Round 3 -- Edit QA run starting (itineraries {start_id}-{end_id})")
