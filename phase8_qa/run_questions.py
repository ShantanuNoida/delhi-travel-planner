...
Agent 3 (Question Command Agent): applies 20 real probing question commands
per itinerary (400 total), via the REAL app code (phase4/intent_classifier +
phase4/explain_engine, which does real RAG lookups against ChromaDB +
real Gemini synthesis) -- no simulated/mocked behavior. Starts from the SAME
base itinerary Agent 1 built (results/base_itinerary_NN.json); no edits are
applied here, mirroring Phase 2's design (isolates question-answering
quality from edit-engine behavior).

Usage: python run_questions.py [start_id] [end_id]   (1-based, inclusive; default 1 20)
Writes results/question_itinerary_<id>.json per itinerary as it finishes.
"""
...
PROGRESS_LOG = os.path.join(RESULTS_DIR, "_progress_questions.log")
...
def _with_retry(fn, *args, **kwargs):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        except Exception as e:  # noqa: BLE001 -- QA harness: retry any transient classifier/RAG/LLM failure
            last_exc = e
...
def load_base_itinerary(spec_id: int) -> dict:
...
def run_one(spec: dict) -> dict:
    ...
    itinerary = load_base_itinerary(spec["id"])

    questions = generate_question_commands(itinerary, spec)

    steps = []
    for q in questions:
        log(f"  [{q['n']:02d}/20] ({q['category']}/{q['probe']}) \"{q['command']}\"")
        ...
        step = {
            "n": q["n"], "command": q["command"], "category": q["category"], "probe": q["probe"],
            "target_stop": q["target_stop"], "expected": q["expected"], "classification": classification,
        }
    ...
    result = {"spec": spec, "itinerary_full": itinerary, "steps": steps}
    out_path = os.path.join(RESULTS_DIR, f"question_itinerary_{spec['id']:02d}.json")
...
def main():
    ...
    log(f"Team Waypoint Round 3 -- Question QA run starting (itineraries {start_id}-{end_id})")
    for spec in ITINERARY_SPECS:
        if not (start_id <= spec["id"] <= end_id):
            continue
        out_path = os.path.join(RESULTS_DIR, f"question_itinerary_{spec['id']:02d}.json")
        if os.path.exists(out_path):
            log(f"Skipping itinerary {spec['id']:02d} (already has a Question result file)")
            continue
        try:
            run_one(spec)
        except Exception:
